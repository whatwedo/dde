function _updateDockerCompose() {
    ...
    if [ "$networksDefaultExternalName" = "dde" ]; then
        ...
        _yq eval 'del(.networks.default.external.name)' --inplace --indent 4 --prettyPrint docker-compose.yml
        ...
        if [ "$(_yq eval '.networks.default.external | length' docker-compose.yml)" = "0" ]; then
            _yq eval 'del(.networks.default.external)' --inplace --indent 4 --prettyPrint docker-compose.yml
        fi
    fi
    ...
    if [ "$networksDefaultExternalName" = "dde_ssh-agent_socket-dir" ]; then
        ...
        _yq eval 'del(.volumes.ssh-agent_socket-dir.external.name)' --inplace --indent 4 --prettyPrint docker-compose.yml
        ...
        if [ "$(_yq eval '.volumes.ssh-agent_socket-dir.external | length' docker-compose.yml)" = "0" ]; then
            _yq eval 'del(.volumes.ssh-agent_socket-dir.external)' --inplace --indent 4 --prettyPrint docker-compose.yml
        fi
    fi
}
