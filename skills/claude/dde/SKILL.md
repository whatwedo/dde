...
Worktree Compose operations use a checkout-scoped `--project-name` that overrides `name:` and `COMPOSE_PROJECT_NAME`; use dde commands for `up`, `down`, `exec`, and `logs` so they address that identity.
