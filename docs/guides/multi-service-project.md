...
- The file declares no `networks:` block and no `dde_ssh-agent_socket-dir` volume — the per-project `dde-services-<project>` (or `dde-services-<project>-<suffix>-<id>` for a worktree) network and the SSH-Agent volume are injected by the overlay that `project:up` generates.
...
- Attached to the per-project `dde-services-<project>` (or `dde-services-<project>-<suffix>-<id>` for a worktree) network
