...
Explicit router `service` references are rewritten along with the corresponding service names. Both unqualified references and `@docker` references follow the worktree; references to other providers, such as `@file` and `@internal`, keep their original targets.
