...
All Compose commands for a worktree use the same explicit checkout-scoped `--project-name`, including the configuration merge and image build. The implicit image tag for a build-only service uses that same name, so shell and entrypoint inspection inspect the image actually built for the worktree. Main-checkout naming follows Compose defaults. See [Compose project identity](../guides/worktrees.md#compose-project-identity).
