...
ENV);
    ...
    public function testGenerateOverrideInjectsClaudeAgentServiceWhenEnabled(): void
    {
        $this->createComposeFile([
        ...
        try {
            $config = ResolvedConfig::merge(
                new GlobalConfig(claudeAgentEnabled: true),
                new ProjectConfig(name: 'myproject'),
            );

            $overridePath = $this->manager->generateOverride($config, $this->tempDir);
            $data = Yaml::parseFile($overridePath, Yaml::PARSE_CUSTOM_TAGS);

            $this->assertArrayHasKey('dde-claude', $data['services']);
    ...
    public function testGenerateOverrideSkipsClaudeAgentWhenDisabled(): void
    {
        $this->createComposeFile([
            'web' => ['image' => 'nginx:latest'],
        ]);
        ...
        $config = ResolvedConfig::merge(
            new GlobalConfig(claudeAgentEnabled: false),
            new ProjectConfig(name: 'myproject'),
        );
        ...
        $this->assertArrayNotHasKey('dde-claude', $data['services']);

        unlink($overridePath);
    }

    public function testGenerateOverrideSkipsClaudeAgentWhenHomeNotSet(): void
    {
        $this->createComposeFile([
            'web' => ['image' => 'nginx:latest'],
        ]);
        ...
        try {
            ...
            if ($savedServerHome !== null) {
                $_SERVER['HOME'] = $savedServerHome;
            }

            if ($savedEnvHome !== null) {
                $_ENV['HOME'] = $savedEnvHome;
            }
        }
    }

    public function testGenerateOverrideUsesConfiguredClaudeImage(): void
    {
        $this->createComposeFile([
            'web' => ['image' => 'nginx:latest'],
        ]);
        ...
        try {
            $config = ResolvedConfig::merge(
                new GlobalConfig(claudeAgentEnabled: true, claudeAgentImage: 'my-registry/my-claude:v2'),
                new ProjectConfig(name: 'myproject'),
            );
            ...
            $this->assertSame('my-registry/my-claude:v2', $data['services']['dde-claude']['image']);
    ...
    public function testBuildClaudeContainerNameMainCheckout(): void
    {
        $this->assertSame('myproject-claude', DockerComposeManager::buildClaudeContainerName('myproject'));
    }

    public function testBuildClaudeContainerNameWorktree(): void
    {
        $worktreeInfo = new WorktreeInfo(
            mainDirectory: '/projects/myproject',
            worktreeDirectory: '/projects/myproject-feature',
            branch: 'feature/test',
            suffix: 'myproject-feature',
        );
